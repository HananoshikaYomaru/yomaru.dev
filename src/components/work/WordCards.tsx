...
export const WorkCard = ({hiddenSm = false , ...work} : CardProps ) => {
  ...
  return (
    <a className={`focus:outline-none col-span-4  flex-col cursor-pointer ${hoverring.root} ${hiddenSm ? " hidden lg:flex " : "flex"}`} href={`/work/${slug}`}>
      <div
        className={`aspect-[3/4] rounded-lg bg-gray-200  ${hoverring.target} border overflow-hidden`}
      >
        <CloudinaryImg publicId={extractPublicId (bannerImage[0].url)  } width={600} height={800} />
      </div>
...
const WorkCards = () => {
  ...
  return (
    <div className="mx-[12vw] mt-24 xl:mt-36">
      ...
      <p className="text-lg xl:text-xl">I list all my project and work experience here. They can come from any context, including course work, personal project, or and a real job. Each article provides an overview of my tasks and learning from these experiences so they are not supposed to dive deep in technical details. If you want to see my learning journey, go to <Link href="/blog"><a className="underline">blog</a></Link>.</p>
